...
router.get('/FRED/statistics/*', function(req, res,next){
    ...
    helper.log(`GET ${code} Request Received`, 'quandl-router @ /api/quandl/FRED/statistics/')
    url = quandl.getFredUrl(code);
    request(url, function(error, response, body){
        if(error){
            helper.warn(error.message, 'quandl-router @ /api/quandl/FRED/statisitcs/')
            res.status(404).end()
        }
        else{
            ...
            helper.log(`GET ${code} Response Sent: {date, ${code}}: {${response_json.date}, ${response_json.value}}`, 
                        "quandl-router @ /api/quandl/FRED/statistics/");
...
router.get('/WIKI/closing-daily-price/*', function(req, res, next){
    ...
    helper.log(`GET ${ticker} Request Received`, 'quandl-router @ /api/quandl/WIKI/closing-daily-price/');
    url = quandl.getWikiUrl(ticker);
    url = quandl.column(url, quandl.columnValues().close);
    request(url, function(error, response, body){
        if(error){
            helper.warn(error.message, 'quandl-router @ /api/quandl/WIKI/closing-daily-price/');
            res.status(404).end()
        }
        else{
            response_json = quandl.formatResponseBody(body)
            helper.log(`GET ${ticker} Response Sent: {date, price}: {${response_json.date}, ${response_json.value}}`, 
                        "quandl-router @ /api/quandl/WIKI/closing-daily-price/");
            res.status(200).send(response_json);
        }
...
router.get('/WIKI/closing-weekly-price/*', function(req, res, next){
    ...
    request(url, function(error, response, body){
        if(error){ 
            helper.warn(error.message, 'quandl-router @ /api/quandl/WIKI/closing-weekly-price/') 
            res.status(404).end();
        }
        else{
            response_json = quandl.formatResponseBody(body)
            helper.log(`GET ${ticker} Response Sent: {date, price}: {${response_json.date}, ${response_json.value}}`, 
                    "quandl-router @ /api/quandl/WIKI/closing-weekly-price/");
            res.status(200).send(response_json);
        }
    });
})
